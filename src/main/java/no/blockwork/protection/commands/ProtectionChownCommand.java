...
public final class ProtectionChownCommand implements CommandExecutor {
    private final Blockwork plugin;
    private final Protection protection;

    public ProtectionChownCommand(final Blockwork pluginInstance, final Protection protectionInstance) {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
            @Override
            public void run() {
                player.sendMessage(ChatColor.GRAY + "Endrer eier av område (" + blocks.size() + " blokker)...");
